...
export default function LoginPage() {
  const { setSuccess, setError } = useApiResponse(); // Access the notification context
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setSuccess("Logged in successfully"); // Success notification
        navigate("/"); // Redirect to the dashboard
      }
    } catch (err) {
      const errorMessage =
        err.response?.data?.message || "An error occurred. Please try again.";
      setError(errorMessage); // Error notification
    } finally {
      setIsLoading(false);
    }
  };
